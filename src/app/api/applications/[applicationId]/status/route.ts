...
export const PATCH = async (
  req: NextRequest,
  { params }: { params: Promise<{ applicationId: string }> }, // ← type as Promise
) => {
  try {
    const user = await getAuthUser();
    const { applicationId } = await params; // ← await params
    ...
    const jobId = app.jobId as string; // ✅ FIX
    ...
    await db.transaction(async (tx) => {
      ...
      if (status === "ACCEPTED") {
        ...
        await tx
          ...
          .where(
            and(
              eq(applications.jobId, jobId), // ✅ FIX
