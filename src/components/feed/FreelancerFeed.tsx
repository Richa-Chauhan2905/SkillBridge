...
import JobCard from "@/src/components/feed/cards/JobCard";
import {
  Loader2,
  Search,
  X,
  ...
  const fetchJobs = useCallback(async (page: number) => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          const safeJobs = (data.jobs || []).filter(
            (job: Job) =>
              job?.id && job?.client?.firstName && job?.client?.lastName,
          );
          ...
          const jobsArray = safeJobs as Job[];
  ...
  const handleApply = async (id: string) => {
    ...
    try {
      ...
      setAppliedJobsMap((prev) => {
        ...
        newMap.set(id, "PENDING" as ApplicationStatus);
        return newMap;
      });
